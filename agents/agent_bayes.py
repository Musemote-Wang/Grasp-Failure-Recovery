...
import torch
from models.Weightnet import WeightPolicy
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
test_policy = WeightPolicy(z_dim=15).to(device)
test_policy.load_state_dict(torch.load("checkpoints/GraspEnv_v3/Weightnet/model_best.pth"))
test_policy.eval()
...
class GraspAgent_bayes:
    ...
    def calculate_w_smaples_net(self, z_sample):
        def pad_z_his(T=15):
            z = np.zeros(T, dtype=np.float32)
            mask = np.zeros(T, dtype=np.float32)
            L = self.env.state['attempt']
            z[:L] = self.env.state['history'][:L, 3] * 25.0
            mask[:L] = 1.0
            return z, mask

        z_his_pad, mask = pad_z_his(T=15)
        z_sample_pad = np.zeros((z_sample.shape[0], 15), dtype=np.float32)
        z_sample_pad[:, :z_sample.shape[1]] = z_sample * 25.0
        z_sample_t = torch.tensor(z_sample_pad, device=device)
        z_his_t = torch.tensor(z_his_pad, device=device)
        mask_t = torch.tensor(mask, device=device)

        return test_policy(z_sample_t, z_his_t, mask_t).detach().cpu().numpy()
    ...
    def update(self, action, force):
        ...
        # w_samples = self.calculate_w_smaples_net(z_samples)
